...
## Purpose
Inspect developer environment, detect PM tool clients (Azure DevOps `az`, Jira `jira`/`acli`, GitHub `gh`, Linear, OCR `tesseract`), seek explicit human authorization before running installation commands, and configure secure authentication via OS credential vaults without writing secrets to `.devweave/`.
