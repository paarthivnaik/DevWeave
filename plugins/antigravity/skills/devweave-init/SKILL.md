...
1. **Layer 1 — Repository Structure & Multi-Repo Topology**:
   - Inspect workspace root for project topology:
     - **Single-Service**: Standard standalone application.
     - **Monorepo / Multi-Service**: Scans `services/*`, `apps/*`, `packages/*`, `pnpm-workspace.yaml`, `lerna.json`, `nx.json`, `.sln`, `docker-compose.yml`, or Kubernetes definitions.
     - **Multi-Repo Linked Workspace**: Discovers external service dependencies, shared event contracts, and API gateway routes.
