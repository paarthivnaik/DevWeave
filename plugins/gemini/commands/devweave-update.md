...
# Gemini CLI DevWeave Update Command (`gemini devweave-update`)
...
2. **Intelligent Repository Detection & 24h TTL Evaluation**:
   ...
   - If current directory is not an initialized DevWeave repository:
     - Log: `"Global DevWeave plugin updated. (Run 'gemini devweave-init' inside a project repository to establish repository intelligence)."`
...
- **When inside an initialized repository**:
  ```text
  ✨ DevWeave Gemini CLI Plugin updated & repository intelligence synchronized.
  ...
  Suggested Next Commands:
    • gemini devweave-modernization-context <WorkItemId> (to begin modernization)
    • gemini devweave-context <WorkItemId> (to begin standard work item)
  ```
- **When outside a repository**:
  ```text
  ✨ DevWeave Gemini CLI Plugin updated globally.

  Plugin Version: 1.2.0 (22 active skills)

  Suggested Next Commands:
    • gemini devweave-init (run inside a project repository to establish repository intelligence)
  ```
